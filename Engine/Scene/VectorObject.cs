...
#warning RecalculateEffectiveOrigins is never called
  ...
  public sealed class Polygon : ISerializable
  {
    ...
    #region LOD & Spline
    /// <summary>Gets whether the polygon has any spline edges.</summary>
    [Browsable(false)]
    ...
    /// <summary>Gets or sets the LOD (level of detail) threshold that determines how many of the points generated by
    /// the subdivision process will be discarded. A higher value will discard more points. A value of zero will
    /// discard no points.
    /// </summary>
    ...
    /// This value even affects shapes that have no spline edges (although in those shapes, every direction change is
    ...
    public void Render()
    {
      ...
      if(texCoordsDirty) // ditto for texture coordinates
      {
    ...
    void GenerateTextureCoordinates()
    {
      ...
      Point[] texCoords = new Point[numSubPoints]; // create a working buffer

      // first find the bounding rectangle of the polygon's vertices
      double x1 = double.MaxValue, y1 = double.MaxValue, x2 = double.MinValue, y2 = double.MinValue;
      for(int i=0; i<texCoords.Length; i++)
      {
        Point pt = subPoints[i].Position;
        if(pt.X < x1) x1 = pt.X;
        if(pt.X > x2) x2 = pt.X;
        if(pt.Y < y1) y1 = pt.Y;
        if(pt.Y > y2) y2 = pt.Y;
